...
class DGCNN(nn.Module):
    ...
    def __init__(
        ...
        print(
            "Building DGCNN will have {} graph convolutions".format(
                len(intermediate_feat_dim)
            )
        )
        self.k = k_neighbors
    ...
    def forward(self, x, transpose_input_output=False, spatial_knn=None, pool="max"):
        ...
        :return: the result of forwarding x to DGCNN
        """
        if transpose_input_output:
            x = x.transpose(2, 1)  # feat-dim first, then objects/points

        intermediate_features = []
        for layer in self.layers:
            x = get_graph_feature(
                x, k=self.k, subtract=self.subtract_from_self, idx=spatial_knn
            )
            x = layer(x)
            x = x.max(dim=-1)[0]
        ...
        if pool == "max":
            x = F.max_pool1d(x, kernel_size=x.shape[-1]).squeeze_(-1)
        return x
