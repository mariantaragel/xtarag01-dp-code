    ...
    def lstm_based_forward(self, tokens, stimuli):
        ...
        for i in range(n_stimuli):
            ...
            if self.geo_encoder_uses_lang:
                grounding_feat = self.stimulus_encoder(
                    grounding_input, tokens
                )  # e.g., when using attention.
            else:
...
def weight_regularization_loss(listener):
    ...
    for p in listener.named_parameters():
        ...
        if (
            "weight" in name
            and ("stimulus_encoder" in name)
            or ("classifier_head" in name)
        ):
            reg_loss += p[1].norm(2)
    return reg_loss
...
def single_epoch_train(
    listener, train_loader, criterion, optimizer, device="cuda", **kwargs
):
...
def ablation_model_one(vocab, shape_latent_dim, context_aware=False):
    ...
    visual_encoder = MLP(
        shape_latent_dim,
        [visual_n_hidden, visual_n_hidden],
        dropout_rate=visual_dropout,
    )
    ...
    language_encoder = TransformerModel(
        len(vocab), d_model, nhead, d_hid, nlayers, language_dropout
    )
    ...
    classifier_head = MLP(
        visual_n_hidden + d_model + context_n_hidden,
        clf_mlp_neurons,
        dropout_rate=clf_head_dropout,
    ...
    if context_aware:
        context_encoder = MLP(
            visual_n_hidden,
            [context_n_hidden, context_n_hidden],
            dropout_rate=visual_dropout,
        )
        model = ContextAwareListener(
            language_encoder,
            visual_encoder,
            context_encoder,
            classifier_head,
        ...
        model = ContextFreeListener(
            language_encoder, visual_encoder, classifier_head, lstm_based=False
        )
    return model
...
def ablation_model_two(vocab, shape_latent_dim, context_aware=False):
    ...
    language_encoder = default_lstm_encoder(
        vocab,
        word_embedding_dim,
        lstm_n_hidden,
    ...
    if context_aware:
        ...
        model = ContextAwareListener(
            language_encoder, visual_encoder, context_encoder, classifier_head
        )
    else:
...
class PointNetEncoder(nn.Module):
    def __init__(self, encoder_conv_layers, visual_n_hidden, visual_dropout):
        ...
        self.mlp = MLP(
            self.encoder_conv_layers[-1],
            [self.visual_n_hidden, self.visual_n_hidden],
            dropout_rate=self.visual_dropout,
        )
...
def ablation_raw_pointnet(vocab, context_aware=False):
    ...
    visual_n_hidden = 128
    visual_dropout = 0.3
    visual_encoder = PointNetEncoder(
        encoder_conv_layers, visual_n_hidden, visual_dropout
    )
...
class DGCNNEncoder(nn.Module):
    def __init__(
        ...
        self.dgcnn = DGCNN(
            in_dim,
            intermediate_feat_dim=dgcnn_intermediate_feat_dim,
            out_dim=dgcnn_out_dim,
...
def ablation_raw_dgcnn(vocab, context_aware=False):
    ...
    dgcnn_intermediate_feat_dim = [
        ...
        32,
        128,
    ]  # [64, 64, 128, 256] (higher performing but slower to train)
    dgcnn_out_dim = 256
    k_neighbors = 12
    visual_encoder = DGCNNEncoder(
        3,
        dgcnn_out_dim,
        dgcnn_intermediate_feat_dim,
        k_neighbors,
