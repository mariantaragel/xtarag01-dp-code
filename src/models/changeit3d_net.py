...
class LatentDirectionFinder(nn.Module):
                ...
                if adaptive_id_penalty != "":
                    if adaptive_id_penalty == "distractor_listening_prob":
                        ...
                        identity_loss = (
                            gamma
                            * distractor_probs
                            * safe_l2(edit_latent, dim=[1], reduce=None)
                        )
    ...
    def evaluate(
        ...
        for batch in dataloader:
            ...
            all_texts.append(tokens.cpu())
            all_input_shapes.append(distractor.cpu())
            all_output_shapes.append(transformed_distractor.cpu())
        ...
        result["listening_loss"] = listening_meter.avg
        result["identity_loss"] = identity_meter.avg
        result["texts"] = torch.cat(all_texts).numpy()
        result["inputs"] = torch.cat(all_input_shapes).numpy()
        result["outputs"] = torch.cat(all_output_shapes).numpy()
...
@torch.no_grad()
def get_transformed_latent_code(
    direction_finder, data_loader, stimulus_index, scales=[1], device="cuda"
):
    ...
    for batch in data_loader:
        ...
        for scale in scales:
            if (
                scale == 0
            ):  # no transformation, just return the input/starting latent code
                transformed = s
            else:
