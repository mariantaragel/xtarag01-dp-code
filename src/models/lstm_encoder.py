...
class LSTMEncoder(nn.Module):
    ...
    def __call__(self, tokens, grounding=None, len_of_sequence=None):
        ...
        if len_of_sequence is None:
            ...
            len_of_sequence_in_cpu = (
                len_of_sequence.clone().detach().cpu()
            )  # convert to CPU for pytorch >= 1.12 & pack_padded_sequence()

        x_packed = pack_padded_sequence(
            w_emb, len_of_sequence_in_cpu, enforce_sorted=False, batch_first=True
        )
