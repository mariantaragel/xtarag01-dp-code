...
def pc_ae_transform_point_clouds(
    pc_ae, direction_finder, data_loader, stimulus_index, scales=[1], device="cuda"
):
    """
    ...
    results = get_transformed_latent_code(
        direction_finder, data_loader, stimulus_index, scales=scales, device=device
    )
...
@torch.no_grad()
def sgf_transform_point_clouds(
    sgf_trainer,
    direction_finder,
    data_loader,
    stimulus_index,
...
@torch.no_grad()
def imnet_transform_point_clouds(
    IMNET, direction_finder, data_loader, stimulus_index, scales=[1], device="cuda"
):
    """See pc_ae_transform_point_clouds"""
