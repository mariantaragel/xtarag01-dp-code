...
from in_out.datasets.shape_net_parts import (
    canonical_part_names_to_relevant_words,
...
def mark_part_reference_in_sentences(
    sentences_tokenized,
    shape_classes,
    ...
    for tokens, shape_class in zip(sentences_tokenized, shape_classes):
        ...
        ref_parts_names = sentence_to_its_referred_canonical_parts(
            tokens, canonical_part_of_class
        )
        ...
        if idx_to_segmentation_part_name is not None:
            ...
            referred_parts_idx.append(
                [name_to_idx[p] - 1 for p in ref_parts_names]
            )  # -1 since enumeration of
