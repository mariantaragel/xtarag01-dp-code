...
def chamfer_dists_on_masked_pointclouds(
    pointclouds_a, pointclouds_b, masks_a, masks_b, bsize, device="cuda"
):
    """
    ...
    for locs in iterate_in_chunks(range(n_items), bsize):
        ...
        mb = ~masks_b[locs]
        a[ma, :] = (
            10e6  # assign a large value to ensure that this points will be matched with themselves
        )
        b[mb, :] = 10e6
...
def difference_in_probability(
    original_probabilities, transformed_probabilities, gt_class_labels
):
