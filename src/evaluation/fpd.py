...
def calculate_activation_statistics(
    pointclouds, model, feature_ids, batch_size=100, device=None, verbose=False
):
...
def calculate_fpd(
    ...
    batch_size=100,
    device="cuda",
    architecture="pointnet_with_default_params",
    verbose=False,
):
    ...
    if architecture == "pointnet_with_default_params":
        model.collect_features = types.MethodType(
            collect_features_on_pnet_classifier_built_with_default_params, model
        )
    else:
        raise NotImplementedError(
            "For a different PC architecture you have to implement the feature extractor yourself."
        )

    m1, s1 = calculate_activation_statistics(
        pointclouds_set_a, model, feature_ids, batch_size, device, verbose=verbose
    )
    m2, s2 = calculate_activation_statistics(
        pointclouds_set_b, model, feature_ids, batch_size, device, verbose=verbose
    )
...
@torch.no_grad()
def collect_features_on_pnet_classifier_built_with_default_params(
    self, device, feature_ids=["maxpool", "fc1", "fc2"]
):
    """
    ...
    if "maxpool" in feature_ids:
        ...
        def hook_1(module, hook_input, hook_output):
            out = F.max_pool1d(hook_output, kernel_size=hook_output.shape[-1]).squeeze_(
                -1
            )
    ...
    if "fc1" in feature_ids:
        ...
        self.decoder[0].register_forward_hook(
            hook_2
        )  # fc1 (linear) is at location [0] of decoder: fc/bnorm/relu
    ...
    if "fc2" in feature_ids:
        ...
        self.decoder[3].register_forward_hook(
            hook_3
        )  # fc2 (linear) is at location [3] of decoder: fc/bnorm/relu
