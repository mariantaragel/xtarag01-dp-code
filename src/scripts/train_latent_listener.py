...
if len(args.restrict_shape_class) > 0:
    ...
    logger.info(
        "Restricting to class(es) {}. Total utterances: {}".format(
...
assert df.source_uid.apply(lambda x: x in shape_to_latent_code).all(), (
    "all loaded stimuli must have a latent code"
)
...
for split in ["train", "val", "test"]:
    ...
    dataset = LanguageContrastiveDataset(
        ndf, to_stimulus_func, n_distractors=1, shuffle_items=shuffle_items
    )
...
for split in ["test", "val"]:
    ...
    evaluation_results[split]["augmented_df_with_predictions"] = augmented_df.copy(
        deep=True
    )
    ...
    missing_context_mask = (augmented_df["hard_context"].isna()) & ~(
        augmented_df.assignmentid == "shapeglot"
    )
    n_missing = missing_context_mask.sum()
    if n_missing > 0:
        logger.info(
            f"Warning: {n_missing} non-shapeglot stimuli do not have context hardness information"
        )
    ...
    for tag in ["using_all_data", "excluding_sg_examples"]:
        if tag == "using_all_data":
            ...
            temp = augmented_df[
                (~augmented_df.hard_context.isna())
                & (augmented_df.assignmentid != "shapeglot")
            ]
        else:
            assert False
        ...
        logger.info(
            f"Average Performance in hard pairs for/when {tag}: {accuracy_on_hard}"
        )
        ...
        logger.info(
            f"Average Performance in easy pairs for/when {tag}: {accuracy_on_easy}"
        )
    ...
    if split != "train":
        ...
        distractor_fraction_in_train = (
            f"{distractor_in_training.sum()} / {len(distractor_in_training)}"
        )
        evaluation_results[split]["distractor_fraction_in_train"] = (
            distractor_fraction_in_train
        )
        logger.info(
...
if hasattr(args, "save_analysis_results") and args.save_analysis_results:
    pickle_data(
        osp.join(args.log_dir, "analysis_of_trained_listener.pkl"), evaluation_results
    )
