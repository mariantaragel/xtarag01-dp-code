...
def cdist(x1, x2, epsilon=1e-16):
    ...
    res = (
        x1_norm - 2.0 * inner_prod + x2_norm.t()
    )  # You need to transpose for broadcasting to be correct.
...
def k_euclidean_neighbors(k, x1, x2, exclude_identity=False, identities=None):
    ...
    if exclude_identity:
        all_cross_pairwise_dists = exclude_identity_from_neighbor_search(
            all_cross_pairwise_dists, identities
        )
    n_dists, n_ids = all_cross_pairwise_dists.topk(
...
def k_cosine_neighbors(k, x1, x2, exclude_identity=False, identities=None):
    ...
    :return: M x k
    """
    all_cross_pairwise_dists = torch.mm(
        normalize(x1, dim=1, p=2), normalize(x2, dim=1, p=2).t()
    )
