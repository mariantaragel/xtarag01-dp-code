...
def df_parallel_column_apply(df, func, column_name):
    ...
    res = d_data.map_partitions(
        lambda df: df.apply((lambda row: func(row[column_name])), axis=1)
...
def make_train_test_val_splits(
    df, loads, random_seed, split_column="target", verbose=True
):
    ...
    if verbose:
        print(
            "Using a {},{},{} for train/val/test purposes".format(
    ...
    train, rest = train_test_split(
        unique_ids, test_size=val_size + test_size, random_state=random_seed
    )
    train = set(train)

    if val_size != 0:
        val, test = train_test_split(
            rest, test_size=round(test_size * len(unique_ids)), random_state=random_seed
        )
    else:
        test = rest
    test = set(test)
