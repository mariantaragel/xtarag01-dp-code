...
shapetalk_class_mixing["bottle"] = (
    "bottle",
    None,
    None,
)  # MN "bottle" are duplicates of SN
...
shapetalk_class_mixing["guitar"] = (
    "guitar",
    None,
    None,
)  # MN guitar are duplicates of SN
...
shapetalk_class_mixing["mug"] = (
    "mug",
    None,
    None,
)  # "cup" exists in MN but all duplicates and/or bad alignments
...
shapetalk_class_mixing["table"] = (
    "table",
    ["table", "desk"],
    None,
)  # for tables: in MN we used both table/desk classes, to be similar with SN's "table" category
...
def add_image_files(shape_talk_df, top_img_dir, ending=".png"):
    for model_type in ["source", "target"]:
        ...
        shape_talk_df[new_column] = model_to_file_name(
            shape_talk_df,
...
def model_to_file_name(
    df, model_name_col, object_class_col, dataset_col, top_dir, ending
):
    """
    :param df:
    ...
    def join_parts(row):
        return osp.join(
            top_dir,
            row[object_class_col],
            row[dataset_col],
            row[model_name_col] + ending,
        )
