...
class ShapeNetParts(Dataset):
    ...
    def __init__(self, root_dir, split_df, number_of_points):
        ...
        self.synid_to_shape_cat, self.shape_cat_to_synid = (
            self.synid_to_shape_category_maps()
        )
        ...
        for (
            c,
            m,
        ) in zip(split_df.shape_class, split_df.model_name):
            syn_id = self.category_ids[c]
            pts_file = osp.join(self.root_dir, syn_id, "points", m + ".pts")
            segmentation_file = osp.join(
                self.root_dir, syn_id, "points_label", m + ".seg"
            )
...
idx_to_segmentation_part_name = {
    ...
    "table": {
        ...
        2: "leg",
        3: None,
    },  # 3=> inconsistent: sometimes shelves sometimes connectors...
}


canonical_part_names_to_relevant_words = dict()
canonical_part_names_to_relevant_words["chair"] = {
    "leg": [
        "leg",
        "legged",
        "wheel",
...
canonical_part_names_to_relevant_words["table"] = {
    "leg": ["leg", "legs", "feet", "foot", "footrest", "stretcher", "base", "pedestal"],
    "top": ["top", "apron", "tabletop"],
}

canonical_part_names_to_relevant_words["lamp"] = {
    "floor-base": ["base"],
...
canonical_part_names_to_relevant_words["airplane"] = {
    "body": ["body", "fuselage"],
    ...
    "rudder": [
        ...
        "stabilizer",
        "fin",
    ],  # In this dataset, the "rudder" (label=3) includes all of them
    "engines": ["engine", "engines"],
}

canonical_part_names_to_relevant_words["knife"] = {
    "blade": ["blade"],
    "handle": ["handle", "grip", "handgrip"],
}

canonical_part_names_to_relevant_words["bag"] = {
    "body": ["body"],
    "handle": ["handle", "grip", "handgrip"],
}

canonical_part_names_to_relevant_words["cap"] = {
    "crown": ["crown"],
    "visor": ["visor", "brim"],
}
