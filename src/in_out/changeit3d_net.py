...
def prepare_input_data(args, logger=None):
    ...
    if len(args.restrict_shape_class) > 0:
        ...
        msg = "Restricting to class(es) {}. Total utterances: {}".format(
            args.restrict_shape_class, len(df)
        )
        _print(msg)
    ...
    if args.clean_train_val_data:
        ...
        dataset = LanguageContrastiveDataset(
            df, to_stimulus_func, n_distractors=1, shuffle_items=False
        )
        dataloader = torch.utils.data.DataLoader(
            dataset=dataset, batch_size=args.batch_size, num_workers=args.num_workers
        )
        listening_res = evaluate_listener(
            pretrained_listener, dataloader, device=device, return_logits=True
        )

        # print its test-accuracy.
        listening_acc = (
            (listening_res["logits"].argmax(1) == 1) & (df.listening_split == "test")
        ).sum()
        ...
        drop_mask = (listening_res["logits"].argmax(1) != 1) & (
            df.changeit_split.isin(["train", "val"])
        )
        _print(
            f"Dropping {sum(drop_mask)} examples from train/val because the provided listener does not correctly predict the target for them."
        )
...
def prepare_input_data_loaders(df, shape_to_latent_code, args):
    to_stimulus_func = partial(
        shape_uid_to_stimulus, shape_to_latent_code=shape_to_latent_code
    )
...
def dataloader_for_expression(
    expression, vocab, shape_uids, to_stimulus_func, batch_size=None, num_workers=10
):
    """
...
def shape_with_expression_dataloader_convenient(
    dloader,
    expressions,
    vocab,
    batch_size=None,
    ...
    shape_uids = pd.DataFrame(
        dloader.dataset.df.target[mask_class].unique()
    )  # keep each target once
    shape_uids = shape_uids.assign(key=1)
    shape_uids.columns = ["target", "key"]
    shape_uids = shape_uids.assign(
        target_object_class=shape_uids.target.apply(lambda x: x.split("/")[0])
    )  # convention: class/dataset/name
