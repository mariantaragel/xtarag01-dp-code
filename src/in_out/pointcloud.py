...
def pc_loader_from_npz(
    npz_filename,
    swap_xy_axis=False,
    assert_preprocessed=False,
    only_pc=True,
    n_samples=None,
    random_seed=None,
    pc_dtype=np.float32,
    part_label_dtype=np.int32,
):
    ...
    if n_samples is not None:
        pc, selected_idx = uniform_subsample(
            pc, n_samples=n_samples, random_seed=random_seed
        )
...
def prepare_vanilla_pointcloud_datasets(args):
    ...
    """
    print(
        "Loading ALL pc-data in memory. This can make the deep-net I/O faster but it is optional as it might have a "
    ...
    if args.debug:
        ...
        split_df = split_df.sample(
            min(len(split_df), 1000), random_state=args.random_seed
        )
        split_df.reset_index(inplace=True, drop=True)
    ...
    if not hasattr(args, "n_classes") or args.n_classes is None:
        ...
        if args.n_classes != len(all_train_classes):
            raise ValueError(
                "The number of object classes of the split-file are not equal to the user-provided one."
            )
    ...
    split_df = split_df.assign(
        object_class_int=split_df.object_class.apply(
            lambda x: class_name_to_idx.get(x, -1)
        )
    )
...
def rotate_z_axis_by_degrees(pc, theta, clockwise=True):
    ...
    rotation_matrix = np.array(
        [[cos_t, -sin_t, 0], [sin_t, cos_t, 0], [0, 0, 1]], dtype=pc.dtype
    )
    if not clockwise:
